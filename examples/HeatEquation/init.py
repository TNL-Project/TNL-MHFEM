...
from TNL.MHFEM.mesh_utils import write_mesh_functions


def set_initial_condition(mesh, alpha, beta, gamma):
    numberOfCells = mesh.getEntitiesCount(mesh.Cell)
    vector = pytnl.containers.Array[float]()
...
def set_boundary_condition(mesh, fname_out):
    faces = mesh.getEntitiesCount(mesh.Face)
...
if __name__ == "__main__":
    ...
    argparser = argparse.ArgumentParser(
        description="Heat equation example initial condition generator"
    )
    g = argparser.add_argument_group("input files")
    g.add_argument(
        "--mesh",
        required=True,
        type=argtype_existing_file,
        help="path to file with TNL grid",
    )
    g = argparser.add_argument_group("initial condition parameters")
    g.add_argument(
        "--alpha", type=float, default=-0.05, help="Alpha value in initial condition"
    )
    g.add_argument(
        "--beta", type=float, default=-0.05, help="Beta value in initial condition"
    )
    g.add_argument(
        "--gamma", type=float, default=5, help="Gamma value in initial condition"
    )
    g = argparser.add_argument_group("output files")
    g.add_argument(
        "--output",
        required=True,
        help="where to save the mesh with the initial condition (VTK, VTU or PVTU format)",
    )
    g.add_argument(
        "--output-boundary",
        required=True,
        help="where to save the bundary conditions (TNL format)",
    )

    args = argparser.parse_args()

    mesh_reader, mesh = pytnl.meshes.resolveAndLoadMesh(args.mesh)
